...
require 'yaml'
# require_relative '../app/renderers/customer_renderer'
  ...
  def run(target_path = nil)
    ...
    Dir.glob('config/locales/**/*') do |folder|
      ...
      files.each do |path|
        ...
        next if unsupported_languages.include?(full_dictionary.keys.first)
  ...
  def map_single_language_strings(dictionary_node, output_node)
    dictionary_node.each do |key, object_at_node|
      if object_at_node.is_a?(Hash)
        output_node[key] ||= {}
        map_single_language_strings(object_at_node, output_node[key])
      else
        output_node[key] ||= {}
